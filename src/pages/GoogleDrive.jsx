...
export default function GoogleDrive() {
  ...
  return <div className="drive-page">
    ...
    <PageTitle titulo="Google Drive" descricao="Pastas e documentos da MinasLab, organizados para encontrar e abrir em um clique." acao={editavel && <button className="btn-primary" disabled={ocupado || !itens} onClick={novo}><Plus size={17} />Adicionar atalho</button>} />
    <div className="drive-intro"><span className="drive-intro-icon"><FolderOpen size={26} aria-hidden="true" /></span><div><h2>A central de arquivos da equipe</h2><p>Salve os atalhos aqui. O acesso aos arquivos continua seguindo as permissões do Google Drive.</p></div></div>
    {erro ? <ErroModulo mensagem={erro} aoTentar={() => setTentativa(t => t + 1)} /> : itens === null ? <CarregandoModulo /> : <>
      <div className="drive-summary" aria-label="Visões dos atalhos"><button onClick={() => { setDestaques(false); setArea(''); }} aria-pressed={!destaques && !area}><span>Todos os atalhos</span><strong>{itens.length}</strong></button><button onClick={() => { setDestaques(true); setArea(''); }} aria-pressed={destaques}><span><Star size={15} aria-hidden="true" /> Em destaque</span><strong>{itens.filter(i => i.destaque).length}</strong></button><div><span>Áreas com atalhos</span><strong>{areas.length}</strong></div></div>
      {itens.length > 0 ? <>
      ...
      </> : <div className="drive-empty"><span className="drive-empty-icon"><LinkIcon size={29} aria-hidden="true" /></span><h2>Seus arquivos começam por um atalho</h2><p>{editavel ? 'Quando tiver o link, adicione a pasta ou o documento. Depois, a equipe encontra tudo por aqui.' : 'Os atalhos adicionados pela equipe aparecerão aqui, organizados por área.'}</p>{editavel && <button className="btn-primary" onClick={novo}><Plus size={17} />Adicionar primeiro atalho</button>}<div className="drive-examples" aria-label="Áreas disponíveis">{['Laboratório', 'Administração', 'Licitações', 'Marketing'].map(a => <span key={a}>{a}</span>)}</div></div>}
    </>}
