...
export default function ConfigFinanceiroHub(){const navigate=useNavigate();return <div className="space-y-5"><PageTitle titulo="Configurações Financeiras" descricao="Escolha o cadastro ou integração que deseja alterar. Cada item abre em uma tela própria."/><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{itens.map(({id,titulo,descricao,icone:Icone,rota})=><button key={id} type="button" onClick={()=>navigate(rota)} className="group rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-md"><div className="flex items-start justify-between gap-4"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-teal-50 text-teal-700"><Icone size={20}/></span><ArrowRight size={18} className="mt-2 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-teal-600"/></div><h2 className="mt-2 font-semibold text-slate-900">{titulo}</h2><p className="mt-1 text-sm leading-5 text-slate-500">{descricao}</p></button>)}</div></div>}
