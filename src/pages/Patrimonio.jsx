...
function Indicador({titulo,valor,descricao,icone:Icone,aoClicar,ativo}) {
  return <button className={`pat-metric ${ativo?'is-active':''}`} onClick={aoClicar} disabled={!aoClicar}><span className="pat-metric-top">{titulo}<Icone size={19}/></span><strong>{valor}</strong><span>{descricao}</span></button>;
}
function Impressao({tipo,bens,setores,contexto,aoVoltar}) {
  return <div className="pat-print">
    ...
    {tipo==='etiquetas' ? <div className="pat-labels">{bens.map(b=><div className="pat-label" key={b.id}><small>MINASLAB · PATRIMÔNIO</small><strong>{b.codigo || 'Sem etiqueta'}</strong><span>{b.nomeGenerico}</span><small>{b.descricaoTecnica || nomeSetor(b,setores)}</small></div>)}</div> : <>
      <header className="pat-report-title"><span>MINASLAB · CONTROLE PATRIMONIAL</span><h1>Inventário de bens</h1><p>{contexto}</p><p>Emitido em {dataLonga(ymdLocal(new Date()))} · {bens.length} bens · Valor de aquisição informado: {moedaCheia(bens.reduce((s,b)=>s+b.valor,0))}</p><small>Valores de aquisição, sem depreciação ou avaliação de mercado. {bens.filter(b=>!(b.valor>0)).length} {bens.filter(b=>!(b.valor>0)).length===1?'bem sem valor informado':'bens sem valor informado'}.</small></header>
...
export default function Patrimonio() {
  const sessao=getSessao(), editavel=podeEditar(sessao);
  const abasPermitidas=[['inventario','Inventário'],['setores','Por setor'],['pendencias','Pendências']].filter(([id])=>podeVerSecao('patrimonio',id,sessao));
  const podeInventario=abasPermitidas.some(([id])=>id==='inventario');
  ...
  const [filtros,setFiltros]=useState(FILTROS_INICIAIS), [aba,setAba]=useState(()=>abasPermitidas[0]?.[0]||'inventario'), [modo,setModo]=useState('cards');
  ...
  const recortar=(patch)=>{if(!podeInventario)return;setFiltros({...FILTROS_INICIAIS,...patch});setPagina(1);setSelecionados([]);setAba('inventario');};
  ...
  async function semear() {
    if(!window.confirm(`Cadastrar os ${SETORES_PADRAO.length} setores sugeridos para o MinasLab?`))return;
    setSalvando(true);
  ...
  return <div className="patrimonio-page">
    {erro && <div role="alert" className="pat-notice is-error">{erro}<button className="btn-ghost" onClick={carregar}>Tentar novamente</button></div>}
    <header className="pat-heading"><div><span className="pat-eyebrow">EQUIPAMENTOS E BENS DA EMPRESA</span><h1>Patrimônio</h1><p>Saiba o que temos, onde está e o que precisa de atenção.</p></div><div className="pat-actions"><button className="btn-ghost" disabled={atualizando||salvando} onClick={()=>{carregar();carregarFotos();}} aria-label="Atualizar patrimônio"><RefreshCw size={18} className={atualizando?'animate-spin':''}/></button>{editavel&&<button className="btn-primary" disabled={!vm.setores.length} onClick={()=>abrirBem()}><Plus size={18}/>Novo bem</button>}</div></header>
    <div className="pat-metrics">
      <Indicador titulo="Bens ativos" valor={numero(vm.kpis.quantos)} descricao={`${vm.kpis.baixados} no histórico de baixas`} icone={Boxes} aoClicar={podeInventario?()=>recortar({}):undefined}/>
      <Indicador titulo="Investido em bens" valor={moedaCheia(vm.kpis.valor)} descricao={`${moedaCheia(vm.kpis.noAno)} adquiridos em ${hoje.slice(0,4)}`} icone={Wallet} aoClicar={podeInventario?()=>recortar({ordem:'valor'}):undefined}/>
      <Indicador titulo="Em manutenção" valor={vm.ativos.filter(b=>b.situacao==='manutencao').length} descricao="Ver equipamentos indisponíveis" icone={Building2} ativo={filtros.situacao==='manutencao'} aoClicar={podeInventario?()=>recortar({situacao:'manutencao'}):undefined}/>
      <Indicador titulo="Cadastros a completar" valor={pendentes.length} descricao={fotos===null?'Conferência parcial: fotos pendentes':'Fotos, localização e documentação'} icone={AlertTriangle} ativo={aba==='pendencias'} aoClicar={abasPermitidas.some(([id])=>id==='pendencias')?()=>{setAba('pendencias');setMsg(null);}:undefined}/>
    </div>
    ...
    {aba==='inventario'&&<>
      ...
      {!visiveis.length?<div className="pat-empty"><Boxes size={36}/><h2>{vm.bens.length?'Nenhum bem neste recorte':'Vamos montar seu inventário'}</h2><p>{vm.bens.length?'Experimente outro setor, situação ou termo de busca.':'Cadastre um equipamento, identifique o setor e adicione suas fotos.'}</p>{(vm.bens.length||editavel)&&<button className="btn-outline" disabled={!vm.bens.length&&!vm.setores.length} onClick={()=>vm.bens.length?recortar({}):abrirBem()}>{vm.bens.length?'Limpar filtros':'Cadastrar primeiro bem'}</button>}</div>:modo==='cards'?<div className="pat-grid">{nestaPagina.map(b=><article key={b.id} className={`pat-asset ${selecionados.includes(b.id)?'is-selected':''}`}>
