...
export default function FinanceiroLayout(){
 ...
 return <div className={classes} onClickCapture={capturarCliqueContextual}>
  <nav className="financeiro-navegacao" aria-label="Menu financeiro">
   {(direcao?[principais,complementares]:[principais.filter(x=>x.to==="/financas/servicos-gerados")]).map((grupo,i)=><div className="financeiro-nav-linha" key={i}>
    {grupo.map(({label,curto,to,end,icon:Icon,descricao})=><NavLink key={to} to={to} end={end} aria-label={label} title={descricao?`${label}: ${descricao}`:label} className={`financeiro-nav-link${ativa(location,label,to)?" financeiro-nav-link-ativo":""}`}><Icon size={18} aria-hidden="true"/><span>{curto||label}</span></NavLink>)}
   </div>)}
  </nav>
  ...
  {direcao&&location.pathname==="/financas/servicos-gerados"&&<section className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-blue-100 bg-blue-50/50 px-3 py-2.5 shadow-sm"><div><p className="text-sm font-bold text-slate-900">Modelo de importação — Serviços Gerados</p><p className="text-xs text-slate-600">Baixe o modelo aprovado, mantenha a aba <b>Serviços</b> e as 20 colunas na mesma ordem e importe o arquivo preenchido nesta tela.</p></div><div className="flex flex-wrap items-center gap-2"><ServicosGeradosPrintButton/><button type="button" className="btn-outline h-9 px-3 py-2 text-xs" onClick={baixarModelo} disabled={baixandoModelo}><Download size={15}/>{baixandoModelo?"Baixando...":"Baixar modelo"}</button></div></section>}
