...
const horasOrigem = d => { const v = d.trabalhadoMin ?? d.trackedMin; return v == null || v === "" ? "—" : duracaoTexto(Number(v)); };
...
export default function FolhaMensal({ pessoas = [], pontoDia = [], competencia, editavel }) {
  ...
  async function fechar() {
    ...
    try {
      ...
      setMensagem(`Folha de ${rotulo(competencia)} salva e preservada.`);
      return folha;
  ...
  return <Card>
    <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        <select aria-label="Pessoa da folha mensal" className="select" value={pessoaId} onChange={e=>setPessoaId(e.target.value)}>
        ...
        <button type="button" className="btn-outline" onClick={()=>window.print()} disabled={!pessoa || !linhas.length}><Printer size={15}/> Imprimir / PDF</button>
        {editavel && <button type="button" className="btn-primary" onClick={fechar} disabled={!pessoa || !linhas.length}><Save size={15}/> Salvar folha mensal</button>}
      </div>
    </div>
    <p className="mb-3 text-xs text-slate-500">Esta folha preserva os horários e totais de origem. Para ver as horas consideradas e concluir o fechamento, use Conferência e Ajustes e fechamento.</p>
    ...
    {!pessoa ? <Empty>Selecione um funcionário para visualizar a folha.</Empty> : <>
      <div className="mb-3 grid gap-2 rounded-xl bg-slate-50 p-3 text-sm md:grid-cols-4">
        <span><strong>Funcionário:</strong> {pessoa.nome}</span><span><strong>Matrícula:</strong> {pessoa.matricula || "—"}</span><span><strong>Empresa:</strong> {pessoa.empresa || "Não informada"}</span><span><strong>Total de origem:</strong> {linhas.some(d => d.trabalhadoMin != null || d.trackedMin != null) ? duracaoTexto(total) : "—"}</span>
      </div>
