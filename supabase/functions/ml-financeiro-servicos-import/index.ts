...
function modelo(){
 ...
 const instrucoes=[["MODELO DE IMPORTAÇÃO — SERVIÇOS GERADOS"],["MinasLab / M Lab — Módulo Financeiro"],[],["COMO USAR"],["1. Preencha os dados na aba Serviços, a partir da linha 2."],["2. Na coluna Empresa informe exatamente MinasLab ou M Lab. Essa informação define a empresa de emissão da OS."],["3. As duas linhas EXEMPLO são ignoradas automaticamente; apague-as antes de importar se desejar."],["4. Não renomeie, não reordene e não exclua colunas. O cabeçalho deve permanecer na linha 1."],["5. Salve em .xlsx e importe em Financeiro > Serviços Gerados."],["6. Em Status de Faturamento use: Aguardando, Pronto para faturar, Faturado, Não faturar ou Cancelado."],["7. Informe Data de Emissão da Nota quando a nota já tiver sido emitida."],["8. Datas inválidas ou 30/11/-0001 são tratadas como sem data."],["9. Em OS já faturadas, a importação só preenche a emissão quando ela estiver vazia."]];
 const exemplos=[CABECALHOS,[1,"Em Aberto","Aguardando","EXEMPLO-001","Boleto bancário, 10 dias","Cliente Exemplo Ltda","00.000.000/0001-00","Sem solicitante","00000001/2026","AM00000001/2026","OS00001/2026","Concluída","01/09/2026","01/09/2026","","11/09/2026","30/11/-0001",150,"MinasLab","Linha de exemplo"],[2,"Pago","Faturado","EXEMPLO-002","Pix ou a vista - 1 dia","Pessoa Exemplo","000.000.000-00","Sem solicitante","00000002/2026","AM00000002/2026","OS00002/2026","Concluída","02/09/2026","02/09/2026","02/09/2026","02/09/2026","02/09/2026",200,"M Lab","Linha de exemplo"]];
 const listas=[["Status Fatura","Status de Faturamento","Status Ordem serviço","Forma pagamento","Empresa"],["Em Aberto","Aguardando","Faturamento","Boleto bancário, 10 dias","MinasLab"],["Pago","Pronto para faturar","Parcelado","Boleto bancário, 30 dias","M Lab"],["Faturamento","Faturado","Concluída","Pix ou a vista - 1 dia",""],["Aguardando fechamento","Não faturar","Laboratorio","Cartão de Credito",""],["Cancelada","Cancelado","Cancelada","Boleto bancário, 2 parcelas",""]];
...
Deno.serve(async req=>{
 ...
 try{
  ...
  let inseridos=0,atualizados=0,semEmpresa=0,reativados=0,conflitosRecuperados=0,preservadosFaturados=0,notasCorrigidas=0,datasEmissaoImportadas=0;
  await emLotes([...unicos.entries()],async([ok,r])=>{
   const f=txt(get(r,"fatura")),d=doc(get(r,"cnpj cpf")),nome=txt(get(r,"cliente"))||"Cliente não informado",ex=porOS.get(ok),ref=rm.get(ok),emissaoPlanilha=dt(get(r,"data de emissao da nota","data de emissao","emissao"));
   if(ex&&estaFaturado(ex)){if(emissaoPlanilha&&!ex.data_emissao){const patchEmissao={data_emissao:emissaoPlanilha,updated_at:now(),updated_by:usuario};const{error}=await sb.from("servicos_gerados").update(patchEmissao).eq("id",ex.id);if(error)throw error;datasEmissaoImportadas++;atualizados++;porOS.set(ok,{...ex,...patchEmissao})}else preservadosFaturados++;return}
   if(ex&&temNotaFiscal(ex)){const patchNota:any={status_faturamento:"FATURADO",updated_at:now(),updated_by:usuario};if(emissaoPlanilha&&!ex.data_emissao){patchNota.data_emissao=emissaoPlanilha;datasEmissaoImportadas++}const{error}=await sb.from("servicos_gerados").update(patchNota).eq("id",ex.id);if(error)throw error;notasCorrigidas++;atualizados++;porOS.set(ok,{...ex,...patchNota});return}
   const empresaTexto=txt(get(r,"empresa")),empresaChave=key(empresaTexto);let empresaPlanilha:string|null=null;
   if(empresaTexto){empresaPlanilha=em.get(empresaChave)||aliasEmpresa.get(empresaChave)||null;if(!empresaPlanilha)throw new Error(`Empresa inválida na OS ${txt(get(r,"ordem de servico"))}: ${empresaTexto}. Use MinasLab ou M Lab.`)}
   const empresa=empresaPlanilha||ex?.empresa_id||pm.get(prefKey(d,nome))||null;if(!empresa)semEmpresa++;
   const sg=statusGL(get(r,"status fatura")),fatPlanilha=statusFaturamento(get(r,"status de faturamento")),venc=dt(get(r,"data de vencimento")),valor=num(get(r,"total excel","total"));let fat="AGUARDANDO",pag="AGUARDANDO";
   if(ref?.paga){fat="FATURADO";pag="PAGO"}else if(ref?.emitida){fat="FATURADO";pag="A RECEBER"}else if(fatPlanilha){fat=fatPlanilha;if(fat==="FATURADO")pag=sg==="PAGO"?"PAGO":"A RECEBER";else if(fat==="CANCELADO")pag="CANCELADO"}else if(sg==="PAGO"){fat="FATURADO";pag="PAGO"}else if(sg==="CANCELADO"){fat="CANCELADO";pag="CANCELADO"}else if(sg==="FATURAMENTO")fat="PRONTO PARA FATURAR";
   const base:any={empresa_id:empresa,os_numero:txt(get(r,"ordem de servico")),codigo_gerencialab:f||null,contrato_proposta:txt(get(r,"contrato"))||null,cliente:nome,cnpj_cpf:d||null,servico:txt(get(r,"amostra referencia"))||null,data_os:dt(get(r,"data de recepcao"))||dt(get(r,"data do agendamento")),valor_original:valor,data_emissao:ex?.data_emissao||ref?.data_emissao||emissaoPlanilha||null,data_vencimento:venc,forma_pagamento:ref?.forma_pagamento||txt(get(r,"forma pagamento"))||null,dados_origem:r,apagado:false,updated_at:now(),updated_by:usuario};
   if(ex){aplicarAutomaticos(base,ex,fat,pag,ref);const{error}=await sb.from("servicos_gerados").update(base).eq("id",ex.id);if(error)throw error;if(ex.apagado)reativados++;atualizados++;porOS.set(ok,{...ex,...base})}
   else{const novo:any={...base,valor_faturar:valor,status_faturamento:fat,status_pagamento:pag,pagamento_origem:ref?.paga?"HISTORICO PLANILHA":null,pagamento_referencia:ref?.referencia||null,pagamento_identificado_em:ref?.data_pagamento?`${ref.data_pagamento}T12:00:00-03:00`:null,referencia_pagamento:ref?.referencia||null,created_by:usuario};const{data,error}=await sb.from("servicos_gerados").insert(novo).select("id,os_numero,empresa_id,faturamento_manual,pagamento_manual,apagado,status_faturamento,status_pagamento,numero_nf,data_emissao").single();if(error){if(error.code!=="23505")throw error;const{data:ja,error:buscaErro}=await sb.from("servicos_gerados").select("id,os_numero,empresa_id,faturamento_manual,pagamento_manual,apagado,status_faturamento,status_pagamento,numero_nf,data_emissao").eq("os_numero",base.os_numero).maybeSingle();if(buscaErro||!ja)throw error;conflitosRecuperados++;if(estaFaturado(ja)){preservadosFaturados++;porOS.set(ok,ja);return}if(temNotaFiscal(ja)){const patchNota={status_faturamento:"FATURADO",updated_at:now(),updated_by:usuario};const{error:updateErro}=await sb.from("servicos_gerados").update(patchNota).eq("id",ja.id);if(updateErro)throw updateErro;notasCorrigidas++;atualizados++;porOS.set(ok,{...ja,...patchNota});return}const patch:any={...base};aplicarAutomaticos(patch,ja,fat,pag,ref);const{error:updateErro}=await sb.from("servicos_gerados").update(patch).eq("id",ja.id);if(updateErro)throw updateErro;if(ja.apagado)reativados++;atualizados++;porOS.set(ok,{...ja,...patch})}else{porOS.set(ok,data);inseridos++}}
  },40);
  await sb.from("servicos_gerados_importacoes").insert({arquivo_nome:txt(body.arquivoNome)||"GerenciaLab Dashboard",lidos:rows.length,inseridos,atualizados,excluidos_regra:excluidos,pendentes_empresa:semEmpresa,detalhes:{sem_os:semOS,unicas:unicos.size,lote:40,reativados,conflitos_recuperados:conflitosRecuperados,preservados_faturados:preservadosFaturados,notas_corrigidas:notasCorrigidas,datas_emissao_importadas:datasEmissaoImportadas,modelo_colunas:20,status_faturamento_planilha:true,empresa_planilha:true,regras:{cnpjs:[...EXC_DOC],faturas:[...EXC_FAT],preservar_faturados:true,nota_fiscal_forca_faturado:true}},criado_por:usuario});
  return out({ok:true,lidos:rows.length,unicas:unicos.size,inseridos,atualizados,excluidosRegra:excluidos,semOS,semEmpresa,reativados,conflitosRecuperados,preservadosFaturados,notasCorrigidas,datasEmissaoImportadas});
