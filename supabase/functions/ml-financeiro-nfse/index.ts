...
function calcularServico(r:any){
  ...
  const valorUnitario=r.valor_unitario===""||r.valor_unitario==null?Number(r.valor_total||0):Number(r.valor_unitario);
  const descontoPercentual=Number(r.desconto_percentual??0);
  const subtotal=quantidade*valorUnitario;
  const valorDesconto=subtotal*(descontoPercentual/100);
  const valorTotal=Number((subtotal-valorDesconto).toFixed(2));
  return {quantidade,valorUnitario,descontoPercentual,subtotal:Number(subtotal.toFixed(2)),valorDesconto:Number(valorDesconto.toFixed(2)),valorTotal};
}

function validarFiscal(r:any,cliente:any){
  const erros:string[]=[];
  const calc=calcularServico(r);
  ...
  if(!(calc.quantidade>0))erros.push("Quantidade do serviço deve ser maior que zero.");
  if(!(calc.valorUnitario>0))erros.push("Valor unitário do serviço deve ser maior que zero.");
  if(calc.descontoPercentual<0||calc.descontoPercentual>100)erros.push("Percentual de desconto deve ficar entre 0 e 100%.");
  if(!(calc.valorTotal>0))erros.push("Valor total da nota deve ser maior que zero.");
...
Deno.serve(async(req)=>{
  ...
  const requisitos={certificado:Boolean(Deno.env.get("MLAB_NFSE_CERT_PFX_B64")),senha:Boolean(Deno.env.get("MLAB_NFSE_CERT_PASSWORD")),inscricaoMunicipal:Boolean(Deno.env.get("MLAB_NFSE_INSCRICAO_MUNICIPAL")),municipio:Boolean(Deno.env.get("MLAB_NFSE_MUNICIPIO_IBGE")),regime:Boolean(Deno.env.get("MLAB_NFSE_REGIME_TRIBUTARIO"))};
  ...
  try{
    ...
    if(empresaErro)throw empresaErro;if(!mlab)return json({erro:"Empresa M Lab não encontrada ou inativa."},409);
    ...
    if(b.action==="verificarCertificado"){const certificado=await verificarA1();return json({ambiente,configurado,requisitos,empresa:{id:mlab.id,nome:mlab.nome,cnpj:mlab.cnpj},certificado,transmissaoAtiva,producaoLiberada:Deno.env.get("MLAB_NFSE_PRODUCAO_LIBERADA")==="SIM"},certificado.valido?200:409)}
    if(b.action==="listar"){const {data,error}=await sb.from("notas_fiscais").select("*,cliente:clientes_financeiro(*)").eq("empresa_id",mlab.id).eq("origem","NFSE_NACIONAL").eq("apagado",false).order("created_at",{ascending:false}).limit(300);if(error)throw error;return json({itens:data||[]})}
    ...
    async function resolverCliente(reg:any){
      ...
      const existente=(todos||[]).find((x:any)=>digits(x.cnpj_cpf)===doc)||null;
    ...
    if(b.action==="preparar"||b.action==="rascunhoSalvar"){
      ...
      const calc=calcularServico(reg);
      const dados={versaoLayout:"v1.01-20260727",servico:{codigo:String(reg.codigo_servico||"").trim(),nbs:String(reg.nbs||"").trim()||null,descricao:String(reg.servico_descricao||"").trim(),quantidade:calc.quantidade,valorUnitario:calc.valorUnitario,descontoPercentual:calc.descontoPercentual,subtotal:calc.subtotal,valorDesconto:calc.valorDesconto,valorTotalItem:calc.valorTotal},tributacao:{aliquotaIss:reg.aliquota_iss===""||reg.aliquota_iss==null?null:Number(reg.aliquota_iss),aliquotaSimplesNacional:Number(reg.aliquota_simples_nacional??2.01),issRetido:Boolean(reg.iss_retido),regimeEspecial:reg.regime_especial||null,ibsCbs:reg.ibs_cbs||null},financeiro:{vencimento:reg.data_vencimento,formaPagamento:reg.forma_pagamento||null},observacao:reg.observacao||null,preparadoEm:new Date().toISOString()};
      ...
      const payload:any={empresa_id:mlab.id,tipo:"SAIDA",cliente_id:cliente.id,cnpj_emitente:mlab.cnpj,nome_emitente:mlab.nome,cnpj_destinatario:cliente.cnpj_cpf,nome_destinatario:cliente.nome,email_destino:cliente.email||null,data_emissao:reg.data_emissao||hoje(),data_vencimento:reg.data_vencimento,valor_total:calc.valorTotal,origem:"NFSE_NACIONAL",status_fiscal:"RASCUNHO",nfse_ambiente:ambiente,nfse_dados:dados,observacao:reg.observacao||null,updated_at:new Date().toISOString()};
      let res:any;if(reg.id){res=await sb.from("notas_fiscais").update(payload).eq("id",reg.id).eq("empresa_id",mlab.id).eq("origem","NFSE_NACIONAL").select().single()}else{res=await sb.from("notas_fiscais").insert(payload).select().single()}
